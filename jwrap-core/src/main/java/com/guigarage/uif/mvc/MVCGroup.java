...
public class MVCGroup<C, V> {
    ...
    private void createActionBinding() {
        for (Method method : AccessControllerUtils.getInheritedDeclaredMethods(controllerClass)) {
            if (method.isAnnotationPresent(ActionMethod.class)) {
                ...
                ActionHandler actionLister = null;
    ...
    public void registerActionListener(ActionHandler listener, String actionId) {
        ...
        for (Field field : fields) {
            if (field.getAnnotation(ViewNode.class) != null) {
                ...
                if (AccessControllerUtils.getPrivileged(field, controller) == null) {
                    ...
                    } else if (viewHandler.isViewNodeDelegate(field.getType())) {
                        Object toInject = findViewNode(id);
                        if (toInject != null) {
                            try {
                                AccessControllerUtils.setPrivileged(field, controller, field.getType().getConstructor(ViewNodeWrapper.class).newInstance(viewHandler.createWrapper(toInject)));
